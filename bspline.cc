...
//DoubleVector BSCurve::intersectWithPlane(const Point3D &p, const Vector3D &n) const {
//  DoubleVector result;
//  BSCurve c = *this;
//  std::cout << "Starting intersection check with plane.\n";
//  
//  for (size_t k = 1; k <= c.n_; ++k) {
//    std::cout << "Checking control point segment: " << k << std::endl;
//    size_t deg = c.basis().degree();
//    const auto &knots = c.basis().knots();
//    
//    double d1 = (c.cp_[k-1] - p) * n;
//    double d2 = (c.cp_[k]   - p) * n;
//    
//    std::cout << "Dot products, d1: " << d1 << ", d2: " << d2 << std::endl;
//    
//    if (d1 * d2 <= 0.0) {
//      std::cout << "Potential intersection found between control points " << k-1 << " and " << k << std::endl;
//      
//      if (d1 * d2 == 0.0 && d1 + d2 != 0.0 &&
//          ((k == 1    && d1 == 0.0) ||
//           (k == c.n_ && d2 == 0.0) ||
...
//      g1 /= deg; g2 /= deg;
//      
//      std::cout << "Averaged parameters g1: " << g1 << ", g2: " << g2 << std::endl;
//      
//      double ratio = d1 == 0.0 && d2 == 0.0
//        ? (g1 == k1 ? 0.0 : (g2 == k2 ? 1.0 : 0.5))
//        : std::abs(d1 / (d1 - d2));
//      double new_knot = g1 * (1.0 - ratio) + g2 * ratio;
//      
//      std::cout << "Calculated new knot: " << new_knot << " with ratio: " << ratio << std::endl;
//      
//      if (std::abs((c.eval(new_knot) - p) * n) < epsilon) {
//        std::cout << "Adding new knot to result: " << new_knot << std::endl;
//        result.push_back(new_knot);
//        for (; k <= c.n_; ++k) {
//          double dd = (c.cp_[k] - p) * n;
//          if (dd != 0.0)
//            break;
//          std::cout << "Skipping degenerate case at control point: " << k << std::endl;
//        }
//      } else {
//        size_t old_size = c.n_;
//        c = c.insertKnot(new_knot, deg - 1);
//        std::cout << "Inserted knot, checking size change, old: " << old_size << ", new: " << c.n_ << std::endl;
//        if (c.n_ != old_size)
//          --k;
//      }
//    }
//  }
//  std::cout << "Finished checking all segments.\n";
//  return result;
//}
